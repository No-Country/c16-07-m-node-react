...
export default function Landing() {
  return (
    <main className="bg-secondary-content min-h-screen w-full text-sky-900">

      <section className="flex items-center justify-center flex-col w-full h-fit bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${banner})` }}>
        <div className="card bg-white shadow-xl my-10 md:my-40 min-w-72 mx-4 md:mx-10">
          <div className="card-body items-center text-center text-sky-900">
            <h2 className="card-title text-3xl md:text-4xl font-extrabold max-w-xs md:max-w-96">Conectando Generaciones</h2>
            <p className="my-4 font-bold">Un puente social para adultos mayores</p>
            <div className="card-actions w-full px-4 md:w-5/6">
              <Link to="/Login" className="btn text-lg md:text-xl bg-amber-500 hover:bg-amber-400 active:bg-amber-300 w-full font-bold">Únete Ahora</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="flex flex-wrap justify-evenly">
        <div className="flex flex-col items-center justify-evenly my-10 min-h-96 md:my-auto px-4 md:px-10 text-center md:min-h-96 md:w-1/2">
          <h2 className="text-3xl md:text-4xl font-bold text-black">Eventos para compartir</h2>
          <p className="text-lg md:text-xl my-4 font-semibold">¡Descubre un mundo de oportunidades emocionantes y conexiones significativas en nuestra sección de eventos! Comparte momentos inolvidables con personas que comparten tus intereses.</p>
          <Link to="/Login" className="btn bg-sky-700 text-white w-64 md:w-80 my-2 hover:bg-sky-500 active:bg-sky-900 font-bold">Quiero explorar los eventos</Link>
        </div>
        <div className="carousel carousel-center md:w-1/2 p-1 space-x-4 bg-sky-900 h-auto md:h-[600px]">
      ...
      <section className="w-full h-fit text-white text-center py-16 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: 'linear-gradient(rgba(12, 74, 110, 0.8), rgba(12, 74, 110, 0.8)), url("../public/assets/image/people_group_landing.png")' }}>
        <div className="text-center px-4">
          <h2 className="text-3xl md:text-4xl font-bold">Conecta, acompaña, acompáñate</h2>
          <p className="text-lg md:text-xl font-semibold p-2">Programa encuentros, salidas o simplemente charlas para brindar o recibir compañía.</p>
        </div>
        ...
        <Link to="/Login" className="btn text-black bg-amber-500 w-64 md:w-80 my-4 hover:bg-amber-400 active:bg-amber-300 font-bold text-lg border-black">Quiero conectar</Link>
      </section>

<section className="text-white w-full h-fit py-10 pb-40">
  <div className="text-center my-6 text-sky-900">
    <h2 className="text-3xl md:text-4xl font-bold m-4">Testimonios</h2>
    <p className="text-lg md:text-xl font-semibold p-2">Conoce la opinión de nuestra comunidad.</p>
  </div>
  <div className="flex flex-wrap justify-center">
    <article className="w-full px-4 md:w-72 bg-sky-500 rounded-xl shadow-2xl m-2">
      ...
      <div className="text-white text-center bg-sky-700 p-5 rounded-xl">
        <p className="text-xl md:text-2xl font-semibold">Juan Pérez</p>
